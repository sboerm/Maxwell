...
#pragma omp single
	...
	/* Orthogonalize with respect to current approximation */
	product1 = massprod_edge2d(e[l], x[l]);
	add_edge2d(-product1, e[l], x[l]);

	/* Normalize */
	product1 = dotprod_edge2d(x[l], x[l]);
	scale_edge2d(1.0/sqrt(product1), x[l]);
	
	/* Set up the Ritz system */
	zero_edge2d(d[l]);
	addeval_edge2d(1.0, 1.0, x[l], d[l]);
	Ar->a[0] = energyprod_edge2d(1.0, 1.0, x[l], x[l]);
	Ar->a[1] = energyprod_edge2d(1.0, 1.0, e[l], x[l]);
	Ar->a[3] = energyprod_edge2d(1.0, 1.0, e[l], e[l]);
	
	/* Solve the Ritz system */
#pragma omp barrier
#pragma omp single
	syev_matrix(Ar, lambdar);
	
	rayleigh = lambdar[0];
	
	/* Set up new eigenvector approximation */
	scale_edge2d(Ar->a[1], e[l]);
	add_edge2d(Ar->a[0], x[l], e[l]);
