...
#pragma omp single
      ...
      /* Reset "old" eigenvector */
      eold = 0;
      
#pragma omp single
      printf("Initial defect %.4e (%.4e)\n", norm, dnorm);

      for(i=0; i<lopcg_steps; i++) {
	/* Apply the multigrid preconditioner */
	zero_edge2d(x[l]);
	for(j=0; j<prec_steps; j++)
	  vcycle_edge2d(l, 2, 0, b, x, d);
	
	/* Eliminate null-space components in update */
	center_edge2d(x[l]);
	...
	if(eold) {
	  if(ortho_gramschmidt) {
	    /* Orthogonalize update with respect to previous vectors */
	    product1 = massprod_edge2d(e[l], x[l]);
	    add_edge2d(-product1, e[l], x[l]);
	    product1 = massprod_edge2d(eold, x[l]);
	    add_edge2d(-product1, eold, x[l]);
	    
	    /* Normalize update vector */
	    product1 = massprod_edge2d(x[l], x[l]);
	    scale_edge2d(1.0/sqrt(product1), x[l]);
	  }
	  else {
#pragma omp single
	    v[0] = x[l];
	    ...
	    orthonormalize_edge2d(3, v, tau, d[l]);
	  }

	  /* Set up the Ritz system */
	  A3->a[0] = energyprod_edge2d(1.0, 1.0, x[l], x[l]);
	  A3->a[1] = energyprod_edge2d(1.0, 1.0, e[l], x[l]);
	  A3->a[2] = energyprod_edge2d(1.0, 1.0, eold, x[l]);
	  A3->a[3] = A3->a[1];
	  A3->a[4] = energyprod_edge2d(1.0, 1.0, e[l], e[l]);
	  A3->a[5] = energyprod_edge2d(1.0, 1.0, eold, e[l]);
	  A3->a[6] = A3->a[2];
	  A3->a[7] = A3->a[5];
	  A3->a[8] = energyprod_edge2d(1.0, 1.0, eold, eold);
	  
	  /* Solve the Ritz system */
#pragma omp barrier
#pragma omp single
	  syev_matrix(A3, lambda3);

	  /* Compute new eigenvector approximation */
	  copy_edge2d(e[l], d[l]);
	  rayleigh = lambda3[0];
	  scale_edge2d(A3->a[1], e[l]);
	  add_edge2d(A3->a[0], x[l], e[l]);
	  add_edge2d(A3->a[2], eold, e[l]);
	  copy_edge2d(d[l], eold);
	  ...
	  /* Re-normalize */
	  product1 = massprod_edge2d(e[l], e[l]);
	  scale_edge2d(1.0/sqrt(product1), e[l]);

	  /* Orthonormalize old approximation */
	  product1 = massprod_edge2d(e[l], eold);
	  add_edge2d(-product1, e[l], eold);
	  product1 = massprod_edge2d(eold, eold);
	  scale_edge2d(1.0/sqrt(product1), eold);
	}
	else {
	  /* Eliminate null-space components in update */
	  center_edge2d(x[l]);
	  ...
	  /* Orthogonalize the update with respect to previous
	   * vectors */
	  product1 = massprod_edge2d(e[l], x[l]);
	  add_edge2d(-product1, e[l], x[l]);
	  ...
	  /* Set up the Ritz system */
	  A2->a[0] = energyprod_edge2d(1.0, 1.0, x[l], x[l]);
	  A2->a[1] = energyprod_edge2d(1.0, 1.0, e[l], x[l]);
	  A2->a[2] = A3->a[1];
	  A2->a[3] = energyprod_edge2d(1.0, 1.0, e[l], e[l]);
	  
	  /* Solve the Ritz system */
#pragma omp barrier
#pragma omp single
	  syev_matrix(A2, lambda3);

	  /* Copy old eigenvector approximation */
#pragma omp single
	  eold = new_edge2d(e[l]->gr);
	  copy_edge2d(e[l], eold);

	  /* Compute new eigenvector approximation */
	  rayleigh = lambda3[0];
	  scale_edge2d(A2->a[1], e[l]);
	  add_edge2d(A2->a[0], x[l], e[l]);
	  ...
	  /* Orthogonalize old eigenvector approximation */
	  product1 = massprod_edge2d(e[l], eold);
	  add_edge2d(-product1, e[l], eold);
	  
	  /* Normalize old eigenvector approximation */
	  product1 = massprod_edge2d(eold, eold);
	  scale_edge2d(1.0/sqrt(product1), eold);
	}
